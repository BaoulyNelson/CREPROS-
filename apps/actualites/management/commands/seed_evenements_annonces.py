...
from apps.actualites.models import Evenement, Annonce
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for data in EVENEMENTS:
            ...
            if created:
                ev_crees += 1
                self.stdout.write(f"  ✔  [{statut:10s}]  {ev.titre[:60]}")
            else:
                self.stdout.write(
        ...
        for data in ANNONCES:
            date_ev = (
                now + data["delta_evenement"] if data.get("delta_evenement") else None
            )
