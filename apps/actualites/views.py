...
# ── Permissions basées sur le rôle de Utilisateur (apps.comptes) ──────────────
# Utilisateur.role vaut "administrateur", "redacteur" ou "membre".
# Rédacteurs et administrateurs peuvent écrire ; seuls les administrateurs gèrent.
...
def utilisateur_peut_gerer(user):
    """Seul un administrateur peut gérer (catégories, tous les articles, etc.)."""
...
class VueTableauBord(EditeurRequisMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        user = self.request.user
        is_gestionnaire = utilisateur_peut_gerer(user)
        articles = (
            Article.objects.all()
            if is_gestionnaire
...
class VueDashboardArticles(EditeurRequisMixin, ListView):
    ...
    def get_queryset(self):
        ...
        qs = (
            Article.objects.all()
            if utilisateur_peut_gerer(user)
            else Article.objects.filter(auteur=user)
        )
