...
class ParametresSite(models.Model):
    ...
    devise = models.CharField(
        "Devise", max_length=255, default="Mieux savoir pour mieux agir."
    )
    ...
    image_hero = models.ImageField(
        "Image de la section d'accueil (Hero)", upload_to="site/", blank=True, null=True
    )
    texte_hero = models.CharField("Texte d'accroche (Hero)", max_length=255, blank=True)
    texte_appel_action = models.CharField(
    ...
    def clean(self):
        if not self.pk and ParametresSite.objects.exists():
            raise ValidationError(
                "Un seul enregistrement de paramètres du site est autorisé. "
...
class ObjectifSpecifique(models.Model):
    """Objectif spécifique du centre, affiché sur la page d'accueil / à propos."""

    titre = models.CharField("Titre", max_length=255)
    description = models.TextField("Description", blank=True)
    icone = models.CharField(
        "Classe icône (Bootstrap Icons)",
        max_length=60,
...
class ValeurOrganisation(models.Model):
    ...
    icone = models.CharField(
        "Classe icône (Bootstrap Icons)", max_length=60, default="bi-star"
    )
    ordre_affichage = models.PositiveIntegerField("Ordre d'affichage", default=0)
...
class Statistique(models.Model):
    ...
    libelle = models.CharField(
        "Libellé", max_length=150, help_text="Exemple : Recherches publiées"
    )
    valeur = models.PositiveIntegerField("Valeur", help_text="Exemple : 25")
    suffixe = models.CharField(
        "Suffixe", max_length=10, blank=True, help_text="Exemple : +, %"
    )
    icone = models.CharField(
        "Classe icône (Bootstrap Icons)", max_length=60, default="bi-graph-up"
    )
    ordre_affichage = models.PositiveIntegerField("Ordre d'affichage", default=0)
