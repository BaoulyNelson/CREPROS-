/**
 * Script principal du site — interactions légères et accessibles.
 */
document.addEventListener('DOMContentLoaded', function () {
    // Activation du lien de navigation correspondant à la page courante.
    const cheminActuel = window.location.pathname;
    document.querySelectorAll('.navbar-nav .nav-link').forEach(function (lien) {
        if (lien.getAttribute('href') === cheminActuel) {
            lien.classList.add('active');
        }
    });

    // Animation discrète d'apparition au défilement.
    const elementsAnimes = document.querySelectorAll('.animer-apparition');
    if ('IntersectionObserver' in window && elementsAnimes.length) {
        ...
        elementsAnimes.forEach(function (element) { observateur.observe(element); });
    }

    // Fermeture automatique des alertes après quelques secondes.
    document.querySelectorAll('.alert').forEach(function (alerte) {
        setTimeout(function () {
            const instance = bootstrap.Alert.getOrCreateInstance(alerte);
            instance.close();
        }, 6000);
    });
});
